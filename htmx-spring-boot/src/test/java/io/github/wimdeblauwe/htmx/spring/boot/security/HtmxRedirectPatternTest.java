...
class HtmxRedirectPatternTest {
    ...
    private static final String HX_LOCATION_TEMPLATE = """
            {
              "path": "%s",
              "headers": {
                "HX-Boosted": "true"
    ...
    void shouldSendHxRedirectOnSuccessfulHxLogin() throws Exception {
        mockMvc.perform(post("/login")
               ...
               .andExpect(header().json(HX_LOCATION.getValue(), HX_LOCATION_TEMPLATE.formatted(LOGIN_SUCCESS_URL)));
    }
    ...
    void shouldSendHxRedirectOnSuccessfulHxLogout() throws Exception {
        mockMvc.perform(post("/logout")
                                ...
                                .with(csrf()))
               .andExpect(status().isOk())
               .andExpect(header().json(HX_LOCATION.getValue(), HX_LOCATION_TEMPLATE.formatted(LOGOUT_SUCCESS_URL)));
    }
    ...
    void shouldSendHxRedirectOnUnauthorizedHxRequest() throws Exception {
        mockMvc.perform(get("/unauthorized")
               ...
               .andExpect(header().json(HX_LOCATION.getValue(), HX_LOCATION_TEMPLATE.formatted(UNAUTHORIZED_URL)));
    }
    ...
    void shouldSendHxRedirectOnForbiddenHxRequest() throws Exception {
        mockMvc.perform(get("/forbidden")
               ...
               .andExpect(header().json(HX_LOCATION.getValue(), HX_LOCATION_TEMPLATE.formatted(FORBIDDEN_URL)));
    }
